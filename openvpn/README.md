...
The present script has been modified for the ADAV (Auto Deploy Anonymous VPN) script collection.

The following lines have been modified in the original source :
Line 562: "bypass-dhcp" have been removed in order for clients to get the DNS IP of Bind configured in OpenVPS

The following lines have been added to the original source :
Line 696: script-security 2
Line 697: dhcp-option DNS 10.8.0.1
Line 698: up /etc/openvpn/update-resolv-conf
Line 699: down /etc/openvpn/update-resolv-conf

Others adds:
An option has been added to select "localhost" for the DNS configuration.

These lines allow Ubuntu clients to apply VPN DNS servers instead of own DNS when connecting to the vpn through "openvpn clientexample.ovpn"
See the README_FIRST file at the root directory of the script collection for further information.
